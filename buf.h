 ...
 * are in bio.c.
 * @note
 * - swap()で操作するフィールドを以下に示す。
 *  - blkno <- swapのarg1
 *  - b_addr b_xmem <- swapのarg2
 *  - b_wcount <- swapのarg3 @todo <<5したあと-=するのはなぜ
 *  - b_blknoは、char*型にしていて、unsigned intとして使いたい
 *
 */
struct buf
{
...
// ブロックデバイス 
// queue
// rktab:devtab --> d_actf head
// rktab:devtab --> d_actl tail
